...
import { createKhataEntry, getProducts, Product, Customer } from "@/lib/supabase";

interface AddKhataEntryFormProps {
  customer: Customer;
  onBack: () => void;
  onSuccess: () => void;
}

export function AddKhataEntryForm({ customer, onBack, onSuccess }: AddKhataEntryFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    products_taken: [] as Array<{ product_id: string; product_name: string; quantity: number }>,
    amount_paid: 0,
    remaining_due: 0,
    bill_photo_url: ""
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to add khata entry. Please try again.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gradient">Add Khata Entry</h2>
          <p className="text-muted-foreground">for {customer.customer_name}</p>
        </div>
      </div>
      ...
      <Card className="card-electric p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button type="submit" className="w-full" disabled={loading}>
            {loading ? "Adding Entry..." : "Add Khata Entry"}
          </Button>
        </form>
      </Card>
    </div>
